# Annex A — Baseline System Comparison

**Document ID:** RADR / ANX-A  
**Version:** 1.2.0  
**Status:** Conceptual

Data: [`data/baseline_systems.json`](../data/baseline_systems.json)

---

## Summary Table

| System | Status | Launcher | Round | Guidance | Warhead | Range | Squad/SOF |
|--------|--------|----------|-------|----------|---------|-------|-----------|
| **RADR** | Concept | ≤ 5.5 kg, 40 in | ≤ 3.5 kg, 60×18 in | IR F&F, moderate-maneuver | Cube cone + pyrotechnic dispersal | **200–1200 m** (1000 m sweet spot) | Yes |
| FIM-92 Stinger | Fielded | ~15 kg | ~10 kg | High-agility IR | Blast-frag | 500–4800 m | Marginal |
| Carl Gustaf M4 | Fielded | ~6.6 kg | ~3.2 kg | Unguided | Multi | 100–1000 m | Yes |
| C-UAS rifle | Fielded | ~4 kg | ~12 g | Unguided | Bullet | 50–400 m | Yes |

---

## RADR Primary Threats vs. Baselines

| Threat | RADR | MG | Stinger |
|--------|------|-----|---------|
| FPV kamikaze | **Designed for** | Limited range/Pk | Overkill / not issued |
| Quadcopters | **Designed for** | Skill-limited | Overkill |
| Loitering munitions | **Designed for** | Short | Possible but heavy |
| Glide / low signature | **IR seeker** | Visual only | Yes (different scale) |

---

[← README](../README.md) | [06 — System Description](../docs/06-system-description.md)
